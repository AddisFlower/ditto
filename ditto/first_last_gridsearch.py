...
def grid_search(bert_importance_attention, model, tokenizer, sts_tasks):
    # constants used to represent necessary paths for SentEval
    PATH_TO_SENTEVAL = './SentEval'
    PATH_TO_DATA = './SentEval/data'
    BERT_IMPORTANCE_ATTENTION_LAYER = bert_importance_attention[0]
    BERT_IMPORTANCE_ATTENTION_HEAD = bert_importance_attention[1]
    ...
    print("------ %s ------" % ('dev'))
    print(f"The importance attention layer utilized was {BERT_IMPORTANCE_ATTENTION_LAYER}")
    print(f"The importance attention head utilized was {BERT_IMPORTANCE_ATTENTION_HEAD}")
    # print(results)
    print(results['dev']['spearman'].correlation * 100)
    print("\n")

    return results['dev']['spearman'].correlation * 100